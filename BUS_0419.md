...
- [`INVARIANTS.md`](./INVARIANTS.md) — SIG-* rules this document implements; BIF-5 (single-carrier attachment) and DISP-* (dispatch isolation) relate.
- [`CONCEPTS.md`](./CONCEPTS.md) §8 — bus static/temporal layering at the vocabulary level.
- [`STEP_MODEL.md`](./STEP_MODEL.md) — the publish phase of the in-step commit discipline invokes bus primitives.
- [`SIGNAL_ATTACHMENTS.md`](./SIGNAL_ATTACHMENTS.md) — per-subsystem publication catalog. *(forthcoming)*
...
Per EXC-1, EXC-2, EXC-3 in INVARIANTS.md, three mechanisms are normatively excluded from bus publication:
...
- [`INVARIANTS.md`](./INVARIANTS.md) §6 (SIG-*), §10 (EXC-*), §2 (BIF-5).
- [`CONCEPTS.md`](./CONCEPTS.md) §8 (bus static/temporal), §9 (wait primitive integration).
- [`STEP_MODEL.md`](./STEP_MODEL.md) §3.4 (publish phase), §3.5 (return).
- [`SUBSYSTEM_ANATOMY.md`](./SUBSYSTEM_ANATOMY.md) §7 (import rules: bus/ restrictions).
- [`SIGNAL_ATTACHMENTS.md`](./SIGNAL_ATTACHMENTS.md) — per-subsystem catalog of wire usage. *(forthcoming)*
- [`ADR-resolution-half.md`](./ADR-resolution-half.md) §14 (reactor; waker infrastructure is reactor-provided).
